...
function countConsistentStrings(allowed, words) {
  // words의 요소 개수를 allowNum으로 선언
  // allowed의 각 철자를 배열로 나눈 것의 각 요소에서 words의 요소에서 일치하지 않는 철자가 있는 경우 allowNum 에서 -1
  // words 배열을 순회하면서 위의 절차 반복
  // 최종적으로 계산된 allowNum을 반환

  const allowedArr = allowed.split("");
  let allowNum = words.length;

  words.forEach((word) => {
    const wordArr = word.split("");
    for (let i = 0; i < wordArr.length; i++) {
      if (!allowedArr.includes(wordArr[i])) {
        allowNum--;
        return;
      }
    }
